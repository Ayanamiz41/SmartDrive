...
const chunkSize = 1024 * 512 * 20;
const fileList = ref([]);
const delList = ref([]);

const addFile = async (file, filePid, deptMode, departmentId, summary) => {
  const fileItem = {
    file: file,
    //文件UID
    uid: file.uid,
  ...
  await checkResumeTask(fileItem);
  uploadFile(md5FileUid, fileItem.chunkIndex);
};
...
// 检查是否有未完成的上传任务（重新上传同一文件时自动续传）
const checkResumeTask = async (fileItem) => {
  try {
    let result = await proxy.Request({
      url: "/file/upload/resume",
      params: {
    ...
    if (result && result.data && result.data.fileId) {
      ...
      fileItem.chunkIndex = result.data.completedChunks;
      fileItem.uploadSize = result.data.completedChunks * chunkSize;
      fileItem.uploadProgress = Math.floor(
        (fileItem.uploadSize / fileItem.totalSize) * 100
      );
    }
  } catch (e) {
    // 无未完成任务，正常从头开始
  ...
  for (let i = chunkIndex; i < chunks; i++) {
    ...
    let uploadResult = await proxy.Request({
      ...
      params: {
        ...
        deptMode: currentFile.deptMode,
        departmentId: currentFile.departmentId,
...
const getFileByUid = (uid) => {
  let file = fileList.value.find((item) => {
    return item.file && item.file.uid === uid;
  });
  return file;
};
</script>
