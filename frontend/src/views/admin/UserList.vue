    ...
    <div class="file-list">
      ...
      >
        <template #avatar="{ index, row }">
          <div class="avatar">
            <Avatar :userId="row.userId" :avatar="row.avatar"></Avatar>
          </div>
        </template>
        ...
        <template #status="{ index, row }">
          <span v-if="row.status == 1" style="color: var(--success)">启用</span>
          <span v-if="row.status == 0" style="color: var(--danger)">禁用</span>
        </template>
